...
func _post_import(scene: Node) -> Object:
	print_rich("[MaterialAssigner] Processing: ", scene.scene_file_path)
...
func _assign_materials(node: Node) -> void:
	if node is MeshInstance3D:
		...
		for i in mi.get_surface_override_material_count():
			...
			if mat:
				print_rich("[MaterialAssigner]   Slot %d: '%s'" % [i, mat.resource_name])
				var replacement := _find_material(mat.resource_name)
				if replacement:
					mi.set_surface_override_material(i, replacement)
					print_rich("[MaterialAssigner]     -> replaced with ", replacement.resource_path)
...
func _search(dir_path: String, target: String) -> Material:
	...
	if not dir:
		print_rich("[MaterialAssigner]   [color=orange]Cannot open[/color] ", dir_path)
		return null
